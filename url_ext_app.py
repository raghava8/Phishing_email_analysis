...
st.title("Email URL Extractor")
st.write("Extract URLs from .eml files or pasted email content.")

option = st.radio("Choose input method:", ["Upload .eml file", "Paste email content"])

email_text = ""

if option == "Upload .eml file":
    uploaded_file = st.file_uploader("Upload .eml file", type=["eml"])
    if uploaded_file:
        try:
            email_text = load_eml_file(uploaded_file.read())
            st.success("Email content loaded successfully.")
...
if st.button("Extract URLs"):
    if not email_text.strip():
        st.warning("Please upload a file or paste email content first.")
    else:
